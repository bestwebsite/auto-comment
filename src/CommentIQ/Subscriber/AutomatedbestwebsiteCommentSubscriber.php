...
class CommentIQ_Subscriber_AutomatedbestwebsiteCommentSubscriber implements CommentIQ_EventManagement_SubscriberInterface
{
    /**
     * The post meta key used to store whether the post has disabled automatic insertion
     * of the bestwebsite comment or not.
     *
     * @var string
     */
    private $disabled_bestwebsite_comment_meta_key;
    ...
    {
        $this->disabled_bestwebsite_comment_meta_key = 'commentiq_disable_bestwebsite_comment';
    ...
    public static function get_subscribed_events()
    {
        return array(
            ...
            'the_content' => 'insert_bestwebsite_comment',
        );
    }

    /**
     * Add an option to the "Discussion" meta box that allows someone to disable
     * automatic insertion of the bestwebsite comment.
     *
     * @param WP_Post $post
     */
    public function add_disable_option(WP_Post $post)
    {
        ...
        echo sprintf($format, $this->disabled_bestwebsite_comment_meta_key, checked($this->is_automatic_insertion_disabled($post), true, false), __('Disable automatic insertion of bestwebsite comment.', 'commentiq'));
    }

    /**
     * Insert the bestwebsite comment into the content of a post about 30%
     ...
     * @return string
     */
    public function insert_bestwebsite_comment($content)
    {
        
        if (!is_singular($this->post_types)
            || has_shortcode($content, CommentIQ_Shortcode_bestwebsiteCommentShortcode::get_name())
        ) {
            return $content;
        }
        ...
        foreach( $content_array as $content_block ) {
            ...
            if (!$inserted && $new_percentage > $percentage_threshold && $old_percentage < $percentage_threshold ) {
                 ...
                 if( preg_match( $regex, $content_block ) ) {
                      $new_content .= $this->bestwebsite_comment_generator->generate($post);
                    $inserted = true;
                 }
    ...
    public function save_disable_option($post_id)
    {
        ...
        if (empty($_POST[$this->disabled_bestwebsite_comment_meta_key])) {
            delete_post_meta($post_id, $this->disabled_bestwebsite_comment_meta_key);
        } elseif ('on' === $_POST[$this->disabled_bestwebsite_comment_meta_key]) {
            update_post_meta($post_id, $this->disabled_bestwebsite_comment_meta_key, 1);
        }
    }

    /**
     * Checks whether the given post has disabled the automatic insertion
     * of bestwebsite Comments.
     *
     * @param WP_Post $post
     *
     * @return bool
    ...
    {
        $disabled = get_post_meta($post->ID, $this->disabled_bestwebsite_comment_meta_key, true);
