...
class CommentIQ_Subscriber_CommentIQAPISubscriber implements CommentIQ_EventManagement_SubscriberInterface
{
    ...
    public function on_comment_edit($comment_id)
    {
        ...
        if ($post instanceof WP_Post) {
            $this->update_bestwebsite_comment($post);
        }
    }
    ...
    /**
     * Compares a potential bestwebsite comment to another WordPress comment.
     *
     * To be considered, a comment must be shorter than 100 characters. If the given
     * comment doesn't match this criteria. The bestwebsite comment is returned. Otherwise, the
     ...
     *
     * @param mixed      $bestwebsite_comment
     * @param WP_Comment $comment
     *
     * @return mixed
     */
    private function compare_comments($bestwebsite_comment, WP_Comment $comment)
    {
        if (!$this->is_valid_comment($comment)) {
            return $bestwebsite_comment;
        }
        ...
        if (!$bestwebsite_comment instanceof WP_Comment) {
            return $comment;
        }

        $bestwebsite_comment_details = get_comment_meta($bestwebsite_comment->comment_ID, $this->comment_details_meta_key, true);

        if ($comment_details['ArticleRelevance'] > $bestwebsite_comment_details['ArticleRelevance']) {
            $bestwebsite_comment = $comment;
        }
    ...
    /**
     * Update the bestwebsite comment for the given post.
     *
     * @param WP_Post $post
     */
    private function update_bestwebsite_comment(WP_Post $post)
    {
        ...
        $bestwebsite_comment = array_reduce(get_comments(array('post_id' => $post->ID)), array($this, 'compare_comments'));
        ...
        if ( $post->post_author === $bestwebsite_comment->user_id && ! $allow_post_author ) {
	        return;
        }

        if ($bestwebsite_comment instanceof WP_Comment) {
            update_post_meta($post->ID, $this->bestwebsite_comment_id_meta_key, $bestwebsite_comment->comment_ID);
        }
    }
}
